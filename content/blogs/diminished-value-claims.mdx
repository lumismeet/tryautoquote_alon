...
Diminished value is the loss in a car's resale or trade-in value that remains after an accident, even when the repair is done perfectly. Once a vehicle has an accident on its history report through services like Carfax or AutoCheck, buyers and dealers treat it as worth less than an identical car with no accident history, regardless of how good the repair actually looks or drives.
...
Start with the car's pre-accident market value, pulled from Kelley Blue Book or NADA using the vehicle's exact trim, mileage, and condition rating. Some drivers use both sources and take the higher figure as their baseline, since insurers will sometimes select whichever valuation produces a lower number for them.
